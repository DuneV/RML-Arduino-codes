...
void loop() {
  if(Serial.available()> 0 ){
    ...
    // Comando de parada a los motores
    Serial.print("Letra recibida: ");
    Serial.println(incomingChar);
    
    if (incomingChar == 'a'){
    // Calcular velocidades necesarias (suponiendo que delta es en grados y tiempo en segundos)
    float speedX = 100;
    float speedY = 100;
    float speedZ = 100;

    // Enviar comandos de velocidad a los motores
    sendSpeedCommand(0, speedX);
    sendSpeedCommand(1, speedY);
    sendSpeedCommand(2, speedZ);
    }
    else {
